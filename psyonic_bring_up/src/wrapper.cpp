...
AHWrapper::AHWrapper(const uint8_t &hand_addr, const uint32_t &b_rate, const std::string &device)
    : hand(hand_addr), baud_rate(b_rate), device_name_(device) {}
...
int AHWrapper::connect() {
  ...
  if (!device_name_.empty()) {
    // Use explicit device name
    rc = connect_serial(device_name_.c_str(), baud_rate);
  } else {
    // Fallback to auto-detection
    rc = autoconnect_serial(baud_rate);
  }

  if (rc != 0) {
    printf("Failed to connect to Hand %d on %s\n",
           hand.address,
           device_name_.empty() ? "auto-detect" : device_name_.c_str());
    return 1;
  } else {
    printf("Connected to Hand %d on %s\n",
           hand.address,
           device_name_.empty() ? "auto-detect" : device_name_.c_str());
    return 0;
  }
...
/* Takes in an array of command values, an enum command, and a reply mode.
